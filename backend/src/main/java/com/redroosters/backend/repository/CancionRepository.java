...
public interface CancionRepository extends JpaRepository<Cancion, Long> {
    ...
    // Cancioon anterior
    @Query("SELECT MAX(c.id) FROM Cancion c WHERE c.id < :id")
    Long findAnteriorId(@Param("id") Long id);

    // Cancioon siguiente
    @Query("SELECT MIN(c.id) FROM Cancion c WHERE c.id > :id")
    Long findSiguienteId(@Param("id") Long id);
}
